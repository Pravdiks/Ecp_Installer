...
public partial class MainWindow : Window, INotifyPropertyChanged
{
    ...
    private string? _settingsPath;
        private bool _autoPasswordArmed;
    ...
    private async void InstallAll_Click(object sender, RoutedEventArgs e)
    {
        ...
        // Перед стартом установки ещё раз кладём пароль в буфер обмена,
        // чтобы авто-вставка в окнах КриптоПро брала именно его, а не
        // предыдущие пути/текст, которые пользователь вставлял до этого.
        if (_autoPasswordArmed && !string.IsNullOrWhiteSpace(password))
        {
            try
            {
                System.Windows.Clipboard.SetText(password);
                _logger.Info("Пароль контейнера записан в буфер обмена перед установкой для авто-вставки в окнах КриптоПро.");
            }
            catch (Exception ex)
            {
                _logger.Warn($"Не удалось записать пароль в буфер обмена перед установкой: {ex.Message}. Авто-вставка может не сработать.");
            }
        }
        ...
        _installService.SetSoftwareRng();
        try
        {
            var done = 0;
            var total = installable.Count;
            await Task.Run(async () =>
            {
                foreach (var task in installable)
                {
                    ...
                    await Dispatcher.InvokeAsync(() => Progress.Value = pct);
                }
            });
    ...
    private void OpenRegistryContainers_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            const string regeditSettingsPath = @"Software\Microsoft\Windows\CurrentVersion\Applets\Regedit";
            const string targetKey = @"HKEY_CURRENT_USER\Software\Crypto Pro\Settings\Keys";

            using (var regeditKey = Registry.CurrentUser.CreateSubKey(regeditSettingsPath))
            {
                regeditKey?.SetValue("LastKey", targetKey, RegistryValueKind.String);
            }
            ...
            _logger.Info("Открыт regedit на HKCU\\Software\\Crypto Pro\\Settings\\Keys.");
        }
        catch (Exception ex)
        {
            _logger.Error($"Не удалось открыть ветку реестра контейнеров: {ex.Message}");
            System.Windows.MessageBox.Show(
                "Не удалось открыть ветку реестра контейнеров. Откройте вручную regedit и перейдите к HKCU\\Software\\Crypto Pro\\Settings\\Keys.",
                "Ошибка",
                MessageBoxButton.OK,
                MessageBoxImage.Error);
        }
    ...
    private void UsePasswordButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            _logger.Warn("Поле «Пароль контейнера/PFX» пустое — нечего запоминать.");
            System.Windows.MessageBox.Show(
                this,
                "Сначала введите пароль в поле «Пароль контейнера/PFX».",
                "Пароль не задан",
                MessageBoxButton.OK,
        ...
        _autoPasswordArmed = !_autoPasswordArmed;

        if (_autoPasswordArmed)
        {
            UsePasswordButton.Background = new SolidColorBrush(System.Windows.Media.Color.FromRgb(0xC5, 0xE1, 0xA5)); // слегка зелёный
            UsePasswordButton.Foreground = System.Windows.Media.Brushes.DarkGreen;
            UsePasswordButton.Content    = "Использовать пароль (вкл.)";
            _logger.Info("Автоматическое использование пароля для окон КриптоПро включено.");

            // Один раз кладём текущий пароль в буфер обмена, чтобы CryptoPro можно было заполнять Ctrl+V
            // (и вручную, и автоматически).
            try
            {
                System.Windows.Clipboard.SetText(PasswordBox.Text);
                _logger.Info("Пароль контейнера записан в буфер обмена для автоматической вставки.");
            }
            catch
            {
                _logger.Warn("Не удалось записать пароль в буфер обмена. Автовставка может не сработать.");
            }
        }
        else
        {
            UsePasswordButton.Background = new SolidColorBrush(System.Windows.Media.Color.FromRgb(0xFF, 0xF3, 0xE0)); // слегка оранжевый
            UsePasswordButton.Foreground = System.Windows.Media.Brushes.DarkOrange;
            UsePasswordButton.Content    = "Использовать пароль";
            _logger.Info("Автоматическое использование пароля для окон КриптоПро отключено.");
        }
    }

    private async void DataGrid_MouseDoubleClick(object sender, MouseButtonEventArgs e)
    {
        var task = TasksDataGrid.SelectedItem as SignatureTask;
        if (task == null) return;
        var needPasswordRetry =
            (task.Status == SignatureTaskStatus.Error &&
             !string.IsNullOrEmpty(task.Message) &&
             task.Message.Contains("пароль", StringComparison.OrdinalIgnoreCase))
            ||
            (task.Status == SignatureTaskStatus.Success && task.HasPrivateKey == false);
        if (!needPasswordRetry)
            return;

        var newPassword = ShowPasswordInputDialog(
            task.DisplayName,
            PasswordBox.Text,
            out var rememberPassword);
        ...
        try
        {
            await _installService.InstallAsync(
                ...
                containerFolder,
                _autoPasswordArmed);
            _logger.Info("Повторная установка выбранной ЭЦП завершена.");
        }
        finally
        {
            _installService.RestoreRng();
    ...
    /// <summary>Показывает диалог ввода пароля. Пароль подставляется из поля «Пароль контейнера/PFX». Возвращает введённый текст или null при отмене.</summary>
    private string? ShowPasswordInputDialog(string displayName, string currentPasswordFromRow, out bool rememberPassword)
    {
        var remember = true;

        var dialog = new Window
        {
            Title = "Пароль контейнера/PFX",
            Width = 440,
            Height = 220,
        ...
        {
            Text = $"Неверный пароль для «{displayName}». Введите пароль (подставлен из поля «Пароль контейнера/PFX»).",
        ...
        {
            Text = currentPasswordFromRow ?? string.Empty,
            MinWidth = 380,
        ...
        {
            Content = "Запомнить пароль в настройках и использовать при следующем запуске",
            IsChecked = true,
            Margin = new Thickness(0, 0, 0, 4)
        };
        rememberCheck.Checked += (_, _) => remember = true;
        rememberCheck.Unchecked += (_, _) => remember = false;
        Grid.SetRow(rememberCheck, 2);
        ...
        grid.Children.Add(rememberCheck);
        grid.Children.Add(buttonsPanel);

        dialog.Content = grid;
        dialog.Activated += (_, _) => { tb.Focus(); tb.SelectAll(); };

        var result = dialog.ShowDialog() == true ? tb.Text : null;
        rememberPassword = result != null && remember;
        return result;
    }
    ...
    private void LoadSettings()
    {
        ...
        PasswordBox.Text = settings.ContainerPassword ?? string.Empty;
    ...
    private void SaveSettings()
    {
        ...
        var settings = new AppSettings
        {
            ...
            ContainerPassword = PasswordBox.Text
        };
